...
EXP_NAME="${EXP_NAME:-pi05_libero_lora_chunk_progress_current_relative_multilayer}"

INIT_CKPT_BASE="${INIT_CKPT_BASE:-/data/Embobrain/openpi/checkpoints/pi05_libero_lora/libero_lora_finetune_256/29999}"
INIT_CKPT="${INIT_CKPT:-$INIT_CKPT_BASE/params}"
...
PROGRESS_LOSS_WEIGHT="${PROGRESS_LOSS_WEIGHT:-1}"
PROGRESS_RELATIVE_LOSS_WEIGHT="${PROGRESS_RELATIVE_LOSS_WEIGHT:-0.25}"
PROGRESS_TARGET_MODE="chunk"
PROGRESS_READOUT_MODE="chunk_current_relative_multilayer"
PROGRESS_INPUT="current(multilayer_prefix_pooled+clean_action_summary), relative(future_low_noise_suffix_tokens)"
PROGRESS_HEAD_ARCH="current: 3x(prefix2048->96) + action_summary128 -> 224 -> 128 -> 1; relative: 288 -> 192 -> 96 -> 1"
PROGRESS_LOW_NOISE_T="0.001"
...
echo "Progress head arch:          $PROGRESS_HEAD_ARCH"
echo "Progress low-noise t:        $PROGRESS_LOW_NOISE_T"
...
uv run python scripts/train_chunk_progress_head_only.py   --config-name "$CONFIG_NAME"   --exp-name "$EXP_NAME"   --init-ckpt "$INIT_CKPT"   --num-train-steps "$TRAIN_STEPS"   --batch-size "$BATCH_SIZE"   --fsdp-devices "$FSDP_DEVICES"   --num-workers "$NUM_WORKERS"   --log-interval "$LOG_INTERVAL"   --save-interval "$SAVE_INTERVAL"   --val-interval "$VAL_INTERVAL"   --val-num-batches "$VAL_NUM_BATCHES"   --val-split-ratio "$VAL_SPLIT_RATIO"   --progress-loss-weight "$PROGRESS_LOSS_WEIGHT"   --progress-relative-loss-weight "$PROGRESS_RELATIVE_LOSS_WEIGHT"   --progress-readout-mode "$PROGRESS_READOUT_MODE"   --overwrite   $WANDB_FLAG
